...
class Foo(wxpg.PGCellRenderer):

    pass


class BaseComponentProperty(wxpg.PGProperty):

    def __init__(self, label, name, value):
        super().__init__(label, name)
        self.SetValue(PyObjectPropertyValue(value))
...
class Vec3Property(BaseComponentProperty):
    ...
    def __init__(self, label, name=wxpg.PG_LABEL, value=None):
        ...
        self.AddPrivateChild(wxpg.FloatProperty('X', value=self.m_value.data[0]))
        self.AddPrivateChild(wxpg.FloatProperty('Y', value=self.m_value.data[1]))
        self.AddPrivateChild(wxpg.FloatProperty('Z', value=self.m_value.data[2]))


class ConnectionProperty(wxpg.PGProperty):

    def __init__(self, label, name=wxpg.PG_LABEL, value=None):
        super().__init__(label, name)
        self.SetValue(PyObjectPropertyValue(value))

        for i, foo in enumerate(self.m_value.data):
            print(foo)
            self.AddPrivateChild(wxpg.StringProperty(str(i), value=foo))

    def ValueToString(self, value, flags):

        #print('\n'.join(value.data),value)
        return ''#'\n'.join(value.data)
...
class TestPanel( wx.Panel ):
    ...
    def __init__( self, parent ):
        ...
        #
        # pg.Append(IntProperty2("IntProperty2", value=[102.4]))
        p = Vec3Property("Vec3Property", value=pc.Vec3(0.0, 1.1, 2.1))
        print(p.GetName())
        pg.Append(p)
        p2 = ConnectionProperty("foobar", value=['foo', 'bar', 'baz'])
        print(p2.GetName())
        pg.Append(p2)

        #pg.SetPropertyEditor("Vec3Property", "LargeImageEditor")
        #pg.SetPropertyEditor("foobar", "SampleMultiButtonEditor")
    ...
    def OnPropGridChange(self, event):
        ...
        if p:
            ...
            print('Changed to ', (p.GetName(), p.GetValue()))
...
class HelloFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #
        # self.frame = wx.Frame()
        self.panel = TestPanel(self)#, "Simple wxPython App")
        ...
        self.bs1.Add(self.panel, 1, wx.EXPAND)
